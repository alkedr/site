...
#define __DEFINE_STRUCT_FOREACH(R, DATA, ITEM) __f(this->__DEFINE_STRUCT_REMOVE_TYPE ITEM);
...
template<class __F> void forEachField(__F __f) {BOOST_PP_SEQ_FOR_EACH(__DEFINE_STRUCT_FOREACH, ~, BOOST_PP_VARIADIC_TO_SEQ(__VA_ARGS__))}   \
template<class __F> void forEachField(__F __f) const {BOOST_PP_SEQ_FOR_EACH(__DEFINE_STRUCT_FOREACH, ~, BOOST_PP_VARIADIC_TO_SEQ(__VA_ARGS__))}   \
